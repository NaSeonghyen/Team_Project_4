...
<body>
	...
	<!-- 20211101 위재림 공지사항(관리자) 상세 보기 수정 -->
	<section>
              			...
              			<div class="board_table_body">
				        	<div class="item">
			                	...
			                	<div class="tit">
									...
									<a href="Ot4zoServlet?command=notice_viewAdmin&nId=${ndvo.nId}&page=${nPageDvo.currentPageNum }&pageDataCount=${nPageDvo.pageDataCount}">${ndvo.nTitle}</a>
								</div>
		...
		<div class="page-list">
			<ul>
				<li> <a href ="Ot4zoServlet?command=notice_listAdmin&page=${nPageDvo.firstPageNum }&pageDataCount=${nPageDvo.pageDataCount}&f=${param.f}&keyword=${param.keyword}" class="button first">&lt;&lt;</a></li>
				<li> <a href ="Ot4zoServlet?command=notice_listAdmin&page=${nPageDvo.prevPageNum }&pageDataCount=${nPageDvo.pageDataCount}" class="button prev">&lt;</a></li>
				<c:forEach var="i" begin="${nPageDvo.startPageNum }" end="${nPageDvo.endPageNum }" step="1">
					<c:choose>
						<c:when test="${i eq nPageDvo.currentPageNum }">
							<li> <a href ="Ot4zoServlet?command=notice_listAdmin&page=${i}&pageDataCount=${nPageDvo.pageDataCount}&f=${param.f}&keyword=${param.keyword}" class="btn num">${i}  </a> </li>
						</c:when>
						<c:otherwise>
							<li> <a href ="Ot4zoServlet?command=notice_listAdmin&page=${i}&pageDataCount=${nPageDvo.pageDataCount}&f=${param.f}&keyword=${param.keyword}" class="btn num">${i} </a> </li>    
						</c:otherwise>
				...
				<li> <a href ="Ot4zoServlet?command=notice_listAdmin&page=${nPageDvo.nextPageNum }&pageDataCount=${nPageDvo.pageDataCount}" class="button next">&gt;</a></li>
				<li> <a href ="Ot4zoServlet?command=notice_listAdmin&page=${nPageDvo.lastPageNum }&pageDataCount=${nPageDvo.pageDataCount}" class="button last">&gt;&gt;</a></li>
			</ul>
		</div>
	</section>
	<!-- Footer  -->
